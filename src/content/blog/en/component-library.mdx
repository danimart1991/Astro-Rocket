...
**Header** — fixed top navigation bar with logo, nav links, the colour-mode pill (System / Light / Dark dropdown), and an optional CTA button. Three rendering variants: `solid`, `transparent`, and `default`. The capsule header on the homepage is a separate `shape="floating"` prop — distinct from the variant. Includes the scroll progress bar when enabled.
...
**ThemeModeDropdown** — pill-shaped 3-state colour-mode dropdown (System / Light / Dark) backed by `localStorage`. It appears in the header and is re-rendered inside the mobile menu below the `md` breakpoint. See [the colour-mode post](/blog/colour-mode-system) for the full design.
...
Every component uses the design system's CSS color tokens — `--color-brand-500`, `--color-foreground`, `--color-background`, and so on. Change the active theme import and every button, badge, progress bar, and focus ring updates together. No component has a hardcoded brand color.
