...
```html
<html lang="en" class="dark scroll-smooth" data-theme="blue" data-theme-mode="system"></html>
```
...
- `data-theme="..."` identifies the active theme id on `<html>`.
...
- Colour theme selection is code-driven in `src/styles/tokens/colors.css`.
...
The favicon is auto-generated from the first letter of `siteConfig.name` and the active `--brand-500` colour. No image file is needed. To replace it with a custom SVG, drop your file into `public/` and update the path:
...
Astro Rocket ships with **12 colour themes**. Theme selection is done in code by changing the imported theme file in `src/styles/tokens/colors.css`.

### Changing the active theme

Open `src/styles/tokens/colors.css` and set the theme import to the theme you want:
...
If your chosen theme file scopes tokens under a specific `html[data-theme="..."]` selector, make sure `data-theme` in `src/layouts/BaseLayout.astro` matches that id.
...
The active colour theme should be treated as a configuration choice in source, not a runtime UI choice. Use `src/styles/tokens/colors.css` to control which theme file is active for all visitors.
...
```css
--surface-invert:           /* main dark surface background */ --surface-invert-secondary:
  ...
  --on-invert-muted: /* muted text on dark surface */ --border-invert: /* border on dark surface */
...
4. Import your new theme file from `src/styles/tokens/colors.css`.
...
<!-- Single row: logo, nav, social, copyright -->
...
| Active colour theme                  | `src/styles/tokens/colors.css` | Theme `@import`                                       |
...
| All colour theme files               | `src/styles/themes/`           | One CSS file per theme                                |
