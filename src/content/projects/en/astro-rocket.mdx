...
**Astro Rocket** is my own free, open-source theme, built on **Astro 7** and **Tailwind CSS v4**. It isn't a boilerplate you spend a week wiring up — it's a finished website. Every page is already designed and built, so you change the text, set your colour theme in code, and go live.
...
The whole point was a starter that scores **100 on all four Lighthouse categories — Performance, Accessibility, Best Practices, and SEO — out of the box**, on both mobile and desktop. And not on a stripped-back demo page: the full site, with the animations, colour mode toggle, typed headline, and contact form all running.
...
## Twelve themes, code-first

Astro Rocket ships **twelve colour themes** — Orange, Amber, Lime, Emerald, Teal, Cyan, Sky, **Blue**, Indigo, Violet, Purple, and Magenta. Pick one in code via `src/styles/tokens/colors.css` and the entire UI follows that token set: buttons, links, logo badge, blog-cover gradients, and focus states.

Underneath it's CSS custom properties scoped to the active theme, so the browser does the work and the server never has to think about it.
